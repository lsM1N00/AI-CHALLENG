...
class LangGraphNodes:
    ...
    async def _generate_cross_feedback(self, current_agent: str, current_response: dict, other_responses: dict, query: str) -> dict:
        ...
        try:
            ...
            # 피드백 생성 프롬프트
            feedback_prompt = f"""
다음은 사용자 질문과 다른 에이전트들의 답변입니다. 당신의 전문 분야 관점에서 이들 답변에 대한 건설적인 피드백을 제공하세요.
...
"""
    ...
    async def _refine_response_with_feedback(self, agent_type: str, original_response: dict, feedback_data: dict, query: str) -> dict:
        ...
        try:
            ...
            # 답변 보완 프롬프트
            refinement_prompt = f"""
다음은 당신이 이전에 제공한 답변과 다른 에이전트들로부터 받은 피드백입니다. 
피드백을 검토하고 더 완성도 높은 답변으로 보완해주세요.
...
**보완 지침**:
1. 피드백에서 지적된 누락된 정보를 추가하세요
2. 부정확한 부분이 있다면 수정하세요
3. 다른 관점의 정보를 적절히 통합하세요
4. 원본 답변의 강점은 유지하면서 개선하세요
5. 더 포괄적이고 정확한 답변을 제공하세요
