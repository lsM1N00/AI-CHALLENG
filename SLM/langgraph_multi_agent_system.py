...
class LangGraphMultiAgentSystem:
    ...
    def __init__(self):
        ...
        print("[LangGraphMultiAgentSystem] 시스템 초기화 완료 (체크포인터 활성화)")
    ...
    def _get_or_create_thread_id(self, user_id: str = "default") -> str:
        """사용자별 스레드 ID 생성 또는 가져오기"""
        if user_id not in self.conversation_threads:
            self.conversation_threads[user_id] = f"thread_{user_id}_{int(time.time())}"
            self.system_stats["active_conversations"] += 1
        
        return self.conversation_threads[user_id]
    
    async def process_user_query(self, user_query: str, user_id: str = "default") -> Dict[str, Any]:
        """사용자 쿼리를 LangGraph 워크플로우로 처리 (대화 내용 저장 포함)"""
        ...
        try:
            ...
            # 2. 사용자별 스레드 ID 생성
            thread_id = self._get_or_create_thread_id(user_id)
            print(f"[LangGraphMultiAgentSystem] 사용자 스레드 ID: {thread_id}")
            ...
            # 4. 체크포인터 설정으로 LangGraph 워크플로우 실행
            print("[LangGraphMultiAgentSystem] 워크플로우 실행 중...")
            config = {"configurable": {"thread_id": thread_id}}
            
            final_state = None
            
            # 체크포인터를 사용하여 상태를 순차적으로 업데이트하며 실행
            ...
            # 7. 대화 내용을 체크포인터에 저장
            self._save_conversation_to_checkpointer(thread_id, user_query, processed_response)
    ...
    def _save_conversation_to_checkpointer(self, thread_id: str, user_query: str, response: Dict[str, Any]):
        """대화 내용을 체크포인터에 저장"""
        try:
            # 체크포인터에 대화 내용 저장
            conversation_data = {
                ...
                "ai_response": response.get('final_answer', ''),
                "execution_time": response.get('execution_time', 0),
                "quality_score": response.get('response_quality', {}).get('overall_score', 0),
                "sources": response.get('consolidated_sources', [])
            }
            
            # 체크포인터에 저장 (실제로는 LangGraph가 자동으로 관리)
            print(f"[Checkpointer] 대화 내용 저장 완료 - 스레드 ID: {thread_id}")
            
        except Exception as e:
            print(f"[Checkpointer] 대화 내용 저장 실패: {e}")
    ...
    def get_conversation_history(self, user_id: str = "default") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            thread_id = self.conversation_threads[user_id]
            
            # 체크포인터에서 대화 히스토리 조회
            try:
                # 체크포인터의 get 메서드를 사용하여 스레드 데이터 조회
                thread_data = self.checkpointer.get({"configurable": {"thread_id": thread_id}})
                
                if thread_data and isinstance(thread_data, dict):
                    # 스레드 데이터에서 대화 내용 추출
                    conversations = []
                    
                    # 각 노드의 결과를 순회하며 대화 내용 수집
                    for node_name, node_result in thread_data.items():
                        if isinstance(node_result, dict) and 'messages' in node_result:
                            messages = node_result['messages']
                            if messages:
                                # 사용자 메시지와 AI 응답을 찾아서 대화 내용 구성
                                user_msg = None
                                ai_msg = None
                                ...
                                if user_msg and ai_msg:
                                    conversations.append({
                                        "timestamp": time.time(),  # 실제로는 메시지 타임스탬프 사용
                                        "user_query": user_msg,
                                        "ai_response": ai_msg,
                                        "node": node_name
                                    })
                    ...
                    print(f"[Checkpointer] 스레드 데이터를 찾을 수 없음: {thread_id}")
                    return []
                    
            except Exception as e:
                print(f"[Checkpointer] 스레드 데이터 조회 실패: {e}")
                return []
            
        except Exception as e:
            print(f"[Checkpointer] 대화 히스토리 조회 실패: {e}")
            return []
    ...
    def clear_conversation_history(self, user_id: str = "default"):
        ...
        try:
            if user_id in self.conversation_threads:
                ...
                # 체크포인터에서 해당 스레드 삭제
                try:
                    # 체크포인터의 delete 메서드를 사용하여 스레드 삭제
                    self.checkpointer.delete({"configurable": {"thread_id": thread_id}})
                    print(f"[Checkpointer] 스레드 삭제 완료: {thread_id}")
                except Exception as e:
                    print(f"[Checkpointer] 스레드 삭제 실패: {e}")
                
                # 로컬 스레드 정보도 삭제
                del self.conversation_threads[user_id]
                self.system_stats["active_conversations"] -= 1
                
                print(f"[Checkpointer] 대화 히스토리 삭제 완료 - 스레드 ID: {thread_id}")
            
        except Exception as e:
            print(f"[Checkpointer] 대화 히스토리 삭제 실패: {e}")
    
    def get_all_conversation_threads(self) -> List[Dict[str, Any]]:
        """모든 대화 스레드 정보 조회"""
        try:
            threads_info = []
            
            for user_id, thread_id in self.conversation_threads.items():
                try:
                    # 각 스레드의 기본 정보 조회
                    thread_data = self.checkpointer.get({"configurable": {"thread_id": thread_id}})
                    ...
                    thread_info = {
                        "user_id": user_id,
                        "thread_id": thread_id,
                        "has_data": thread_data is not None,
                        "created_at": thread_id.split('_')[-1] if '_' in thread_id else "unknown"
                    }
                    
                    threads_info.append(thread_info)
                    
                except Exception as e:
                    print(f"[Checkpointer] 스레드 {thread_id} 정보 조회 실패: {e}")
                    threads_info.append({
                        "user_id": user_id,
                        "thread_id": thread_id,
                        "has_data": False,
            ...
            print(f"[Checkpointer] 모든 스레드 정보 조회 실패: {e}")
            return []
    
    def export_conversation_data(self, user_id: str = "default", format: str = "json") -> str:
        """사용자별 대화 데이터 내보내기"""
        try:
            if user_id not in self.conversation_threads:
            ...
            if format.lower() == "json":
                ...
                output = []
                output.append(f"사용자 ID: {user_id}")
                output.append(f"스레드 ID: {thread_id}")
                output.append(f"내보내기 시간: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(export_data['export_timestamp']))}")
                output.append("=" * 50)
                
                for i, conv in enumerate(history, 1):
                    output.append(f"\n{i}. 대화 내용:")
                    output.append(f"   질문: {conv.get('user_query', 'N/A')}")
                    output.append(f"   답변: {conv.get('ai_response', 'N/A')}")
                    output.append(f"   노드: {conv.get('node', 'N/A')}")
                
                return "\n".join(output)
            else:
                return f"지원하지 않는 형식입니다: {format}"
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parallel_execution": {
                "enabled": self.system_stats.get("parallel_execution_enabled", False),
                "max_agents": self.system_stats.get("max_parallel_agents", 0),
                "execution_count": self.system_stats.get("parallel_execution_count", 0),
                "current_status": "active" if self.system_stats.get("parallel_execution_enabled", False) else "disabled"
            },
