...
class ManagerAgent:
    ...
    def __init__(self):
        ...
        self.llm_handler = LLMHandler(
            model_name="LEESangM1N/Qwen3-4B-general-fintune",
            temperature=0.3
        )
    ...
    async def _generate_final_answer(self, 
        ...
        synthesis_prompt = f"""
# Role: 당신은 모든 에이전트가 가지고 있는 금융, 법률 ,일반상식에 대한 전문지식을 보유한 관리자입니다. 모든 AI 에이전트의 응답을 분석하고 종합하여, 최종 답변을 생성합니다. 또한, 대응 방안, 권리 보호 방안, 분쟁 해결 절차를 명확하고 신뢰성 있게 안내합니다. 150자 내로 정리하고 요약해서 작성하세요.

# Action:
- 아래 사용자 질문과 각 에이전트의 응답, 일관성 분석 결과를 종합적으로 분석하고 핵심적인 사실, 대응 방안, 권리 보호 방안, 분쟁 헤결 절차를 포함한 통합된 최종 답변을 생성합니다.

# Constraints:
- 최종 답변은 정리하고 요약해서 작성합니다.
- 반드시 최종 답변은 150자가 넘지 않게 작성합니다.
- 최종 답변은 최대한 간결하게 작성합니다.
...
[구체적인 해결 방안 그리고 권리 보호를 위한 이행 절차, 구제 절차 및 방법]
