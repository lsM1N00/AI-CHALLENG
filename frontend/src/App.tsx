...
function App() {
  ...
  const loadConsultation = async (messageId: string) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/consultation/${messageId}`);
      const botMessage: Message = {
        id: `bot-${Date.now()}`,
        type: 'bot',
  ...
  const submitSatisfaction = async () => {
    ...
    try {
      ...
      // 2. 서버에 전송 (백그라운드)
      try {
        await axios.post(`${API_BASE_URL}/satisfaction`, satisfactionData);
        console.log('만족도 데이터가 서버에 성공적으로 저장되었습니다.');
      } catch (serverError) {
        console.log('서버 저장 실패, 로컬에만 저장됨:', serverError);
